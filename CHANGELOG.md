...
- 旧evidence blobやimport sourceが消失しても、未検証の表示を残して通常のToDo操作を続けられるようにした。
  新しく記録する証拠は書込み前に検証し、`todo verify`は未検証を報告する。
- 複数taskの旧証拠が不達でも、`evidence promote`で順に再束縛できるようにした。

## 0.69.4 — 2026-09-25

### 修正

- 工場向けruntime errorスナップショットに、製品が保存済みの最終発生版を載せる。
  BugHubが現在の導入版と発生版を区別できるようにする。

## 0.69.3 — 2026-09-22

### 修正

- 完了サイン（intake accept）が、その工程だけの予定外の書き込みを競合として拒んでいたのを直す。
  自分の範囲が増えた分は `added_paths` として記録し、サインは出す。他の工程のファイルと重なる衝突は、これまでどおり止める。

## 0.69.2 — 2026-09-22

### 修正

- DHCPで端末のIPが同一subnet内を移動した時、bridgeの実際の待受と稼働記録・health・生存確認を
  一致させる。正常な中継を故障と誤認して再設定を繰り返す状態を解消し、Hub登録も移動に追従する。
  起動時と稼働中のどちらも、設定IPの手書き更新や手動の停止・再設定を不要にする。
